# we will not use the CUDA package in this script
using Flux
...
function model_loss(encoder, decoder, λ, x, device)
    μ, logσ, decoder_z = reconstruct(encoder, decoder, x, device)
    len = size(x)[end]
    # KL-divergence
...
# function train(; kws...)
    ...
    # args = Args(; kws...)
    args = Args()
    args.seed > 0 && Random.seed!(args.seed) # what does this command do?
    ...
    for epoch = 1:args.epochs
        ...
        # save image
        _, _, rec_original = reconstruct(encoder, decoder, original, device)
